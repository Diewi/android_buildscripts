...
parser.add_argument('-u', '--useprefix', default='', help='project prefix present in the source gerrit')
...
def is_change_picked(change_id, check_picked_count):
        ...
        if 'Change-Id:' in output:
            ...
            for j,t in enumerate(reversed(output)):
                if t == 'Change-Id:':
                    ...
                    break
            if head_change_id.strip() == change_id:
                print('Skipping {0} - already picked in {1} as HEAD~{2}'.format(i, project_path, i))
                return True
    return False
...
if args.topic:
    ...
    url = args.gerrit + '/changes/?q=topic:%s' % args.topic
    if args.verbose:
...
for change in args.change_number:
    ...
    url = args.gerrit + '/changes/?q=%s&o=CURRENT_REVISION&o=CURRENT_COMMIT&pp=0' % change
    if args.verbose:
    ...
    for (i, data) in enumerate(data_array):
        ...
        if (status != 'OPEN' and status != 'NEW' and status != 'DRAFT'):
            if args.force:
                print('!! Force-picking a closed change !!\n')
            else:
                print('Change status is ' + status + '. Skipping the cherry pick.\nUse -f to force this pick.')
                continue

        # Convert the project name to a project path
        #   - check that the project path exists
        if args.useprefix != '':
            project_name = project_name.replace(args.useprefix, '')
        ...
        # Determine the maximum commits to check already picked changes
        found_change = is_change_picked(change_id, 10)
        if not found_change:
            found_change = is_change_picked(change_id, 100)
        if found_change:
            continue
